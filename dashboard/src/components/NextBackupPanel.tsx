...
const buildBackupFilename = (scope: NextBackupScope) => {
	const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
	return `next-${scope}-${timestamp}.rbbackup`;
};
...
export const DashboardBackupControls = ({
	...
	const [exportScope, setExportScope] =
		useState<NextBackupScope>("database");
	const [selectedFile, setSelectedFile] = useState<File | null>(null);
	const [uploadProgress, setUploadProgress] = useState<number | null>(null);
	const backupActionsAvailable = isBinaryRuntime && !runtimeLoading;

	const exportMutation = useMutation(exportNextBackup, {
	...
	const importMutation = useMutation(
		(file: File) => importNextBackup(file, setUploadProgress),
		{
					...
					<ModalBody py={4}>
						<Stack spacing={4}>
							...
							<FormControl isRequired>
								...
								<FileDropzone
									accept=".rbbackup,application/vnd.next.backup,application/gzip"
									isDisabled={
						...
						<Stack spacing={4}>
							...
							<FormControl>
								...
								<Select
									...
									showSearch={false}
									onChange={(event) =>
										setExportScope(event.target.value as NextBackupScope)
									}
								>
