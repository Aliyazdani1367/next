...
export type JSONEditorProps = {
	...
	readOnly?: boolean;
	showToolbar?: boolean;
	canonicalContext?: NextJsonContext;
	...
	(
		...
		const formatJson = useCallback(() => {
			runWithEditorText((text, editor) => {
				...
				setEditorText(editor, stringifyNextJson(parsed, 2, canonicalContext));
			});
		...
		const compactJson = useCallback(() => {
			runWithEditorText((text, editor) => {
				...
				const parsed = text.trim() ? JSON.parse(text) : {};
				setEditorText(editor, stringifyNextJson(parsed, 0, canonicalContext));
			});
		}, [canonicalContext, emitValidation, runWithEditorText, setEditorText]);
			...
			for (const line of highlightLines ?? []) {
				...
				highlightMarkerRefs.current.push(
					session.addMarker(
						...
						`next-json-diff-${highlightVariant}`,
						"fullLine",
						true,
					),
				);
			...
			for (const range of highlightRanges ?? []) {
				...
				highlightMarkerRefs.current.push(
					session.addMarker(
						...
						`next-json-diff-${highlightVariant}-text`,
						"text",
						false,
					),
				);
