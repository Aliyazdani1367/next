...
export type NextBackupScope = "database" | "full";

export interface NextBackupImportResponse {
	scope: NextBackupScope;
...
export interface RuntimeSettingsResponse {
	...
	phpmyadmin_login_mode: "next" | "custom";
...
export const exportNextBackup = async (
	scope: NextBackupScope,
): Promise<Blob> => {
...
export const importNextBackup = async (
	file: File,
	onProgress?: (percent: number) => void,
): Promise<NextBackupImportResponse> => {
	return new Promise((resolve, reject) => {
		...
		xhr.onload = () => {
			if (xhr.status >= 200 && xhr.status < 300) {
				resolve(xhr.response as NextBackupImportResponse);
				return;
			}
