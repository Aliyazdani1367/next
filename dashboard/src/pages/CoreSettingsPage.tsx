...
import {
	canonicalizeNextJson,
	type NextJsonContext,
	stringifyNextJson,
...
const XRAY_LOG_DIR_HINT = "/var/lib/next/xray-core";
...
export const CoreSettingsPage: FC = () => {
	...
	const getAdvancedJson = () => {
		...
		switch (advSettings) {
			case "inboundSettings":
				return stringifyNextJson(cfg.inbounds ?? [], 2, "inbounds");
			case "outboundSettings":
				return stringifyNextJson(cfg.outbounds ?? [], 2, "outbounds");
			case "routingRuleSettings":
				return stringifyNextJson(
					cfg.routing?.rules ?? [],
					2,
					"routingRules",
				);
			default:
				return stringifyNextJson(cfg ?? {}, 2, "config");
		}
	};
	...
	const setAdvancedJson = (value: string) => {
		try {
			...
			switch (advSettings) {
				case "inboundSettings":
					cfg.inbounds = canonicalizeNextJson(parsed, "inbounds");
					break;
				case "outboundSettings":
					cfg.outbounds = canonicalizeNextJson(parsed, "outbounds");
					syncOutboundDisplay(cfg.outbounds as OutboundJson[]);
					break;
				case "routingRuleSettings":
					if (!cfg.routing) cfg.routing = {};
					cfg.routing.rules = canonicalizeNextJson(parsed, "routingRules");
					syncRoutingRuleDisplay(cfg.routing.rules as RoutingRule[]);
					break;
				default: {
					// replace whole config
					const canonicalConfig = canonicalizeNextJson(
						parsed,
						"config",
			...
			form.setValue(
				"config",
				canonicalizeNextJson(cfg, "config") as EditableCoreConfig,
				{
					shouldDirty: true,
				},
			);
	...
	const activeAdvancedJsonMode =
		...
		advancedJsonModes[0];
	const advancedJsonContext: NextJsonContext =
