...
NEXT_REPO="${NEXT_REPO:-nextpanel/Next}"
...
NEXT_RELEASE_REPO="${NEXT_RELEASE_REPO:-nextpanel/Next}"
...
NEXT_BINARY_DEV_MANIFEST_URL="${NEXT_BINARY_DEV_MANIFEST_URL:-}"
NEXT_BINARY_DEV_RELEASE_TAG="${NEXT_BINARY_DEV_RELEASE_TAG:-dev-builds}"
INSTALL_MODE_FILE="$APP_DIR/.install-mode"
CHANNEL_FILE="$APP_DIR/.channel"
BINARY_BIN_DIR="$APP_DIR/bin"
BINARY_SERVER="$BINARY_BIN_DIR/next-server"
BINARY_CLI="$BINARY_BIN_DIR/next-cli"
BINARY_CLI_LAUNCHER="/usr/local/bin/next-cli"
BINARY_METADATA_FILE="$APP_DIR/.binary-release.json"
BINARY_ARTIFACT_PREFIX="${BINARY_ARTIFACT_PREFIX:-next-binaries}"
BINARY_SERVICE_UNIT="/etc/systemd/system/$APP_NAME.service"
CERTBOT_VENV_DIR="$APP_DIR/certbot-venv"
CERTBOT_BIN=""
PARSED_DOMAINS=()
NEXT_SCRIPT_FLAVOR="${NEXT_SCRIPT_FLAVOR:-binary}"
NEXT_SCRIPT_SOURCE_FILE="${NEXT_SCRIPT_SOURCE_FILE:-next-binary.sh}"
NEXT_SCRIPT_INSTALL_PATH="${NEXT_SCRIPT_INSTALL_PATH:-/usr/local/bin/next}"
NEXT_MYSQL_CONFIG_ROOT="${NEXT_MYSQL_CONFIG_ROOT:-/etc/mysql}"
...
get_current_next_version() {
    local version=""
    if [ -f "$CHANNEL_FILE" ]; then
        version=$(tr -d '[:space:]' < "$CHANNEL_FILE")
    fi
    if [ -z "$version" ] && [ -f "$COMPOSE_FILE" ]; then
        version=$(grep -E "image:.*nextpanel/next:" "$COMPOSE_FILE" | head -n 1 | sed -E 's/.*nextpanel\/next:([^"[:space:]]+).*/\1/')
    fi
    printf '%s\n' "${version:-unknown}"
}
...
get_docker_container_id() {
    ...
    $compose_cmd -f "$COMPOSE_FILE" -p "$APP_NAME" ps -q next 2>/dev/null | head -n 1
}
...
set_next_source_ref() {
    ...
    if [ "${NEXT_SCRIPT_BASE_URL_EXPLICIT:-0}" != "1" ]; then
        NEXT_SCRIPT_BASE_URL="${NEXT_RAW_BASE}/scripts/next"
    fi
}

set_next_source_for_version() {
    case "${1:-latest}" in
        dev)
            set_next_source_ref "$NEXT_BINARY_DEV_BRANCH"
            ;;
        dev-*)
            set_next_source_ref "$NEXT_BINARY_DEV_BRANCH"
            ;;
        v[0-9]*)
            set_next_source_ref "$1"
            ;;
        *)
            set_next_source_ref "master"
            ;;
    esac
}
...
script_install_mode() {
    case "${NEXT_SCRIPT_FLAVOR:-docker}" in
        docker|dockerized|compose)
            echo "docker"
            ;;
        binary|bin|native)
        ...
        mixed|"")
            ...
            ;;
        *)
            colorized_echo red "Invalid script flavor: $NEXT_SCRIPT_FLAVOR" >&2
            exit 1
            ;;
    esac
}
...
select_install_mode() {
    ...
    local forced_mode
    forced_mode=$(script_install_mode)
    requested_mode=$(normalize_install_mode "${1:-${NEXT_INSTALL_MODE:-}}")

    if [ -n "$forced_mode" ]; then
        if [ -n "$requested_mode" ] && [ "$requested_mode" != "$forced_mode" ]; then
            colorized_echo red "This script is dedicated to ${forced_mode} installs. Use the matching Next script for $requested_mode." >&2
            exit 1
        fi
        echo "$forced_mode"
        return
    ...
    colorized_echo cyan "Select Next installation mode:" >&2
...
select_next_version() {
    ...
    colorized_echo cyan "Select Next release channel for ${install_mode} mode:" >&2
    colorized_echo yellow "  1) latest (stable release)" >&2
    if [ "$install_mode" = "binary" ]; then
        colorized_echo yellow "  2) dev (latest successful binary build from branch ${NEXT_BINARY_DEV_BRANCH})" >&2
    else
        colorized_echo yellow "  2) dev (latest Docker image from branch ${NEXT_BINARY_DEV_BRANCH})" >&2
    fi
    read -r -p "Release channel [1]: " next_version_answer

    case "$next_version_answer" in
        2|dev|Dev)
            echo "dev"
            ;;
...
get_installed_next_channel() {
    ...
    if [ -f "$COMPOSE_FILE" ]; then
        image_tag=$(grep -E "image:.*nextpanel/next:" "$COMPOSE_FILE" | head -n 1 | sed -E 's/.*nextpanel\/next:([^"[:space:]]+).*/\1/')
        if [ -n "$image_tag" ]; then
            echo "$image_tag"
            return
        fi
...
install_next_script() {
    ...
    if [ -n "$source_version" ]; then
        set_next_source_for_version "$source_version"
    elif is_next_installed; then
        set_next_source_for_version "$(get_installed_next_channel)"
    fi
    SCRIPT_URL="$NEXT_SCRIPT_BASE_URL/$NEXT_SCRIPT_SOURCE_FILE"
    temp_script=$(mktemp)
    ui_spinner_run "Downloading Next command script" curl -fsSL "$SCRIPT_URL" -o "$temp_script"
    if head -n 1 "$temp_script" | grep -qi "<!DOCTYPE"; then
        rm -f "$temp_script"
        colorized_echo red "Unexpected HTML response while downloading script"
        exit 1
    fi
    ui_spinner_run "Installing Next command script" install -m 755 "$temp_script" "$NEXT_SCRIPT_INSTALL_PATH"
    rm -f "$temp_script"
    colorized_echo green "next script installed successfully"
}
...
print_database_menu() {
    ...
    local idx
    ui_header "Next Database" "Choose storage backend for binary install"
    for idx in 1 2 3; do
        printf "  "
...
max_input_time=0"
    ...
    for dir in /etc/php/*/fpm/conf.d /etc/php/*/cli/conf.d; do
        [ -d "$dir" ] || continue
        printf "%s\n" "$ini_content" > "$dir/99-next-phpmyadmin-upload.ini" || true
        wrote=1
    done
    if [ "$wrote" = "1" ]; then
...
prepare_external_app_node_hosting() {
    ...
    local install_root="/opt/next/node"
...
enable_host_phpmyadmin() {
    ...
    colorized_echo green "phpMyAdmin is installed and will be served through Next using local php-fpm."
}
...
enable_phpmyadmin() {
    ...
    if ! is_next_installed; then
        colorized_echo red "Next is not installed. Please install Next first."
        exit 1
    fi
...
perform_ssl_issue() {
    ...
    if [ "$has_ip" -eq 1 ]; then
        ...
        if is_next_installed; then
            detect_compose
            if is_next_up; then
                colorized_echo blue "Restarting Next to apply SSL configuration..."
                down_next
                up_next
                colorized_echo green "Next restarted with SSL configuration"
            fi
        fi
    ...
    # Check if Next is installed and running, then restart to apply SSL changes
    if is_next_installed; then
        detect_compose
        if is_next_up; then
            colorized_echo blue "Restarting Next to apply SSL configuration..."
...
prompt_ssl_setup() {
    ...
    if ! ssl_command issue --email "$ssl_email" --domains "$ssl_domains" --provider "$ssl_provider" --non-interactive; then
        colorized_echo yellow "SSL setup skipped due to input/issuance error. You can retry with: next ssl issue"
    fi
}
...
ssl_renew() {
    ...
    # Note: perform_ssl_issue already restarts Next if needed
}
...
ssl_command() {
    ...
    case "$action" in
        ...
        renew)
            ...
            ;;
        *)
            colorized_echo blue "Usage: next ssl <issue|renew> [options]"
            colorized_echo magenta "  Issue domain SSL: next ssl issue --email you@example.com --domains example.com"
            colorized_echo magenta "  Issue public IP SSL: next ssl issue --email you@example.com --ip-address 203.0.113.10"
            colorized_echo magenta "  Issue self-signed IP SSL: next ssl issue --email you@example.com --domains 203.0.113.10 --provider self-signed"
            ;;
    esac
}
...
ensure_script_matches_installed_mode() {
    ...
    if [ "$installed_mode" != "$forced_mode" ]; then
        colorized_echo red "This Next installation is in ${installed_mode} mode, but ${0##*/} is the ${forced_mode} script."
        if [ "$installed_mode" = "binary" ]; then
            colorized_echo yellow "Use the Next binary script."
        else
            colorized_echo yellow "Use the Next Docker script."
        fi
        exit 1
    fi
}
...
send_backup_to_telegram() {
    ...
    local split_dir="/tmp/next_backup_split"
...
backup_cron_command() {
    local script_path="${NEXT_SCRIPT_INSTALL_PATH:-}"
...
backup_command() {
    ...
    local temp_dir="/tmp/next_backup"
    local timestamp=$(date +"%Y%m%d%H%M%S")
    local backup_file="$backup_dir/backup_$timestamp.tar.gz"
    local error_messages=()
    local log_file="/var/log/next_backup_error.log"
    > "$log_file"
    ...
    if [ -n "$db_type" ]; then
        ...
        case $db_type in
            ...
            mysql)
                if [ -n "$container_name" ]; then
                    if ! docker exec "$container_name" mysqldump -u root -p"$MYSQL_ROOT_PASSWORD" next --events --triggers  > "$temp_dir/db_backup.sql" 2>>"$log_file"; then
    ...
    if ! rsync -a --delete --exclude 'xray-core' --exclude 'mysql' --exclude 'logs' "$DATA_DIR/" "$temp_dir/next_data/" >>"$log_file" 2>&1; then
        error_messages+=("Failed to copy Next data files.")
    fi
...
update_core_command() {
    colorized_echo yellow "Master no longer runs a local Xray core. Update Xray from the Nodes page or the next-node installer."
}
...
install_next() {
    ...
    # Fetch releases
    FILES_URL_PREFIX="$NEXT_RAW_BASE"
...
services:
  next:
    image: nextpanel/next:${next_version}
    restart: always
    env_file: .env
    network_mode: host
    volumes:
      - /var/lib/next:/var/lib/next
      - /var/lib/next/logs:/var/lib/next-node
    depends_on:
      mariadb:
  ...
  mariadb:
    ...
    volumes:
      - /var/lib/next/mysql:/var/lib/mysql
    healthcheck:
...
EOF
        ...
        sed -i 's~^\(SQLALCHEMY_DATABASE_URL = "sqlite:////var/lib/next/db.sqlite3"\)~#\1~' "$APP_DIR/.env"


        # Add the MySQL connection string
        #echo -e '\nSQLALCHEMY_DATABASE_URL = "mysql+pymysql://next:password@127.0.0.1:3306/next"' >> "$APP_DIR/.env"

        sed -i 's/^# \(XRAY_JSON = .*\)$/\1/' "$APP_DIR/.env"
        sed -i 's~\(XRAY_JSON = \).*~\1"/var/lib/next/xray_config.json"~' "$APP_DIR/.env"
        ...
        upsert_env_assignment "MYSQL_DATABASE" "next"
        upsert_env_assignment "MYSQL_USER" "next"
        upsert_env_assignment "MYSQL_PASSWORD" "$MYSQL_PASSWORD"
        
        SQLALCHEMY_DATABASE_URL="mysql+pymysql://next:${MYSQL_PASSWORD_URL_ENCODED}@127.0.0.1:3306/next"
...
EOF
        ...
        if [ "$next_version" == "latest" ]; then
            yq -i '.services.next.image = "nextpanel/next:latest"' "$docker_file_path"
        else
            yq -i ".services.next.image = \"nextpanel/next:${next_version}\"" "$docker_file_path"
        fi
        ...
        sed -i 's~\(XRAY_JSON = \).*~\1"/var/lib/next/xray_config.json"~' "$APP_DIR/.env"
        sed -i 's~\(SQLALCHEMY_DATABASE_URL = \).*~\1"sqlite:////var/lib/next/db.sqlite3"~' "$APP_DIR/.env"
    ...
    colorized_echo green "Next's files downloaded successfully"
}
...
get_binary_release_asset_metadata() {
    ...
    if [ "$next_version" = "latest" ]; then
        release_api="https://api.github.com/repos/${NEXT_RELEASE_REPO}/releases/latest"
    else
        release_api="https://api.github.com/repos/${NEXT_RELEASE_REPO}/releases/tags/${next_version}"
    fi

    release_payload=$(curl -fsSL "$release_api") || {
        colorized_echo red "Unable to read Next release metadata: $release_api" >&2
        exit 1
    }

    resolved_tag=$(echo "$release_payload" | jq -r '.tag_name // empty')
    package_asset_name="next-linux-${binary_arch}.tar.gz"
    server_asset_name="next-server-${resolved_tag}-linux-${binary_arch}"
    cli_asset_name="next-cli-${resolved_tag}-linux-${binary_arch}"
    ...
    if [ -n "$package_asset_url" ] && [ "$package_asset_url" != "null" ]; then
        printf 'archive|%s|%s|\n' "${resolved_tag:-$next_version}" "$package_asset_url"
        return
    fi
    ...
    if [ -n "$server_asset_url" ] && [ "$server_asset_url" != "null" ] && [ -n "$cli_asset_url" ] && [ "$cli_asset_url" != "null" ]; then
        printf 'split|%s|%s|%s\n' "${resolved_tag:-$next_version}" "$server_asset_url" "$cli_asset_url"
        return
    fi

    colorized_echo red "No Go binary release assets found for linux-${binary_arch}." >&2
    colorized_echo yellow "Publish next-linux-${binary_arch}.tar.gz or split next-server/next-cli assets for this release." >&2
    exit 1
}

get_binary_dev_manifest_url() {
    if [ -n "$NEXT_BINARY_DEV_MANIFEST_URL" ]; then
        printf '%s\n' "$NEXT_BINARY_DEV_MANIFEST_URL"
        return
    fi
    printf 'https://raw.githubusercontent.com/%s/%s/%s\n' \
        "$NEXT_RELEASE_REPO" \
        "$NEXT_BINARY_DEV_MANIFEST_BRANCH" \
        "$NEXT_BINARY_DEV_MANIFEST_PATH"
}
...
get_binary_dev_manifest_metadata() {
    ...
    selected=$(echo "$manifest_payload" | jq -r \
        ...
        --arg repo "$NEXT_RELEASE_REPO" \
        --arg release_tag "$NEXT_BINARY_DEV_RELEASE_TAG" '
        def legacy_build:
            .latest? as $latest
            | if ($latest | type) == "object" then
                {
                    ...
                    assets: (
                        reduce ($latest.assets[]? | strings) as $name
                          ({};
                            if ($name | startswith("next-" + $arch + "-")) then
                              .[$arch] = {
                                name: $name,
...
get_binary_dev_artifact_metadata() {
    ...
    nightly_workflow="$NEXT_BINARY_WORKFLOW_NAME"
    case "$nightly_workflow" in
        *.yml|*.yaml) ;;
        *) nightly_workflow="${nightly_workflow}.yml" ;;
    esac
    workflow_runs_api="https://api.github.com/repos/${NEXT_RELEASE_REPO}/actions/workflows/${nightly_workflow}/runs"
    workflow_runs_payload=$(curl -fsSLG "$workflow_runs_api" \
        --data-urlencode "branch=${NEXT_BINARY_DEV_BRANCH}" \
    ...
    latest_run_json=$(echo "$workflow_runs_payload" | jq -c --arg branch "$NEXT_BINARY_DEV_BRANCH" '
        .workflow_runs[]?
        | select(.head_branch == $branch and .event == "push" and .conclusion == "success")
    ' | head -n 1)

    if [ -z "$latest_run_json" ]; then
        colorized_echo red "No successful binary dev workflow run was found on branch ${NEXT_BINARY_DEV_BRANCH}." >&2
        exit 1
    fi

    run_id=$(echo "$latest_run_json" | jq -r '.id // empty')
    head_sha=$(echo "$latest_run_json" | jq -r '.head_sha // empty')
    artifacts_api="https://api.github.com/repos/${NEXT_RELEASE_REPO}/actions/runs/${run_id}/artifacts"
    ...
    artifact_name=$(echo "$artifacts_payload" | jq -r --arg preferred "${BINARY_ARTIFACT_PREFIX}-linux-${binary_arch}" --arg arch "linux-${binary_arch}" '
        [
            .artifacts[]?
            | select((.expired | not) and (.name == $preferred or (.name | startswith("next")) and (.name | contains($arch))))
        ]
    ...
    artifact_url="https://nightly.link/${NEXT_RELEASE_REPO}/workflows/${nightly_workflow}/${NEXT_BINARY_DEV_BRANCH}/${artifact_name}.zip"
...
#!/usr/bin/env bash
set -e
export NEXT_ENV_FILE="$ENV_FILE"
export NEXT_APP_DIR="$APP_DIR"
export NEXT_DATA_DIR="$DATA_DIR"
exec "$BINARY_CLI" "\$@"
EOF
...
Environment=NEXT_APP_DIR=$APP_DIR
Environment=NEXT_ENV_FILE=$ENV_FILE
Environment=NEXT_INSTALL_MODE=binary
Environment=NEXT_BINARY_METADATA_FILE=$BINARY_METADATA_FILE
...
install_binary_next() {
    ...
    if [ -n "${NEXT_BINARY_SERVER_OVERRIDE:-}" ] || [ -n "${NEXT_BINARY_CLI_OVERRIDE:-}" ]; then
        ...
        ui_spinner_run "Installing Next custom server binary" install -m 755 "$NEXT_BINARY_SERVER_OVERRIDE" "$tmp_dir/next-server"
        ...
        if [[ "$package_path" == *.zip ]]; then
            ui_spinner_run "Extracting Next dev artifact" unzip -j -o "$package_path" -d "$tmp_dir"
            dev_package_path="$tmp_dir/next-linux-${binary_arch}.tar.gz"
            if [ -f "$dev_package_path" ]; then
                ui_spinner_run "Unpacking Next binary package" tar -xzf "$dev_package_path" -C "$tmp_dir"
            elif ls "$tmp_dir"/next-*.tar.gz >/dev/null 2>&1; then
                ui_spinner_run "Unpacking Next binary package" tar -xzf "$(ls "$tmp_dir"/next-*.tar.gz | head -n 1)" -C "$tmp_dir"
            fi
        elif [[ "$package_path" == *.tar.gz ]]; then
            ui_spinner_run "Unpacking Next binary package" tar -xzf "$package_path" -C "$tmp_dir"
        else
        ...
        IFS='|' read -r binary_source_type resolved_version server_asset_url cli_asset_url < <(get_binary_release_asset_metadata "$next_version" "$binary_arch")
        if [ "$binary_source_type" = "split" ]; then
            ui_spinner_run "Downloading Next server binary" curl -fL "$server_asset_url" -o "$tmp_dir/next-server"
            ui_spinner_run "Downloading Next CLI binary" curl -fL "$cli_asset_url" -o "$tmp_dir/next-cli"
        else
            package_path="$tmp_dir/next-binary.tar.gz"
            ui_spinner_run "Downloading Next binary package" curl -fL "$server_asset_url" -o "$package_path"
            ui_spinner_run "Unpacking Next binary package" tar -xzf "$package_path" -C "$tmp_dir"
        fi
    fi

    if [ ! -f "$tmp_dir/next-server" ] || [ ! -f "$tmp_dir/next-cli" ]; then
        colorized_echo red "Downloaded binary package is incomplete; next-server or next-cli is missing." >&2
        rm -rf "$tmp_dir"
        exit 1
    fi
    ...
    upsert_env_assignment "NEXT_DATA_DIR" "$DATA_DIR"
    ...
    if [ ! -f "$DATA_DIR/xray_config.json" ]; then
        curl -fsSL --retry 3 --retry-delay 2 --retry-all-errors "$NEXT_RAW_BASE/xray_config.json" -o "$DATA_DIR/xray_config.json" 2>/dev/null || {
            rm -f "$DATA_DIR/xray_config.json"
            colorized_echo yellow "No bundled xray_config.json found; Next will use its built-in default."
        }
    fi

    write_binary_release_metadata "${resolved_version:-$next_version}" "$binary_arch" "${artifact_url:-${server_asset_url:-}}"
    echo "binary" > "$INSTALL_MODE_FILE"
    create_binary_service
    rm -rf "$tmp_dir"
    colorized_echo green "Next binary files installed successfully"
}
...
schedule_binary_service_restart() {
    ...
    if command -v systemd-run >/dev/null 2>&1; then
        systemd-run \
            --unit "$unit_name" \
            --collect \
            --description "Next delayed service restart" \
...
follow_next_logs() {
    if is_binary_install; then
        journalctl -u "$APP_NAME.service" -f -o "$(journal_output_format)" --no-pager | format_next_journal_logs
        return
    fi
...
get_configured_database_type() {
    local flavor
    local db_url
    flavor=$(get_env_value "NEXT_DATABASE_FLAVOR")
    case "$flavor" in
...
managed_database_has_replication() {
    local status gtid
    if grep -RhsEi '^[[:space:]]*(log[-_]bin|server[-_]id|gtid[-_]mode|relay[-_]log|replicate[-_]|binlog[-_](do|ignore)[-_]db)[[:space:]]*=' "$NEXT_MYSQL_CONFIG_ROOT" 2>/dev/null | grep -q .; then
        return 0
    fi
...
disable_managed_database_binary_log() {
    ...
    extra_databases=$(mysql_root_command --batch --skip-column-names -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name NOT IN ('information_schema','mysql','performance_schema','sys','next','phpmyadmin')" 2>/dev/null) || {
        colorized_echo yellow "Could not inspect the managed database; binary logging was left unchanged."
        return 1
    }
    if [ "$extra_databases" != "0" ]; then
        colorized_echo yellow "Binary logging was left unchanged because this MySQL/MariaDB instance contains databases not managed by Next."
        return 0
    fi
    ...
    [ -n "${backup_file:-}" ] && rm -f "$backup_file"
    colorized_echo green "Binary logging disabled for Next's dedicated local MySQL/MariaDB service."
}
...
EOF
    ...
    sql_file=$(mktemp)
    cat > "$sql_file" <<EOF
CREATE DATABASE IF NOT EXISTS \`next\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER IF NOT EXISTS 'next'@'127.0.0.1' IDENTIFIED BY '${escaped_password}';
CREATE USER IF NOT EXISTS 'next'@'localhost' IDENTIFIED BY '${escaped_password}';
ALTER USER 'next'@'127.0.0.1' IDENTIFIED BY '${escaped_password}';
ALTER USER 'next'@'localhost' IDENTIFIED BY '${escaped_password}';
GRANT ALL PRIVILEGES ON \`next\`.* TO 'next'@'127.0.0.1';
GRANT ALL PRIVILEGES ON \`next\`.* TO 'next'@'localhost';
...
EOF
    ...
    mysql_password_url_encoded=$(urlencode_value "$MYSQL_PASSWORD")
    upsert_env_assignment "NEXT_DATABASE_FLAVOR" "$database_type"
    upsert_env_assignment "MYSQL_DATABASE" "next"
    upsert_env_assignment "MYSQL_USER" "next"
    upsert_env_assignment "MYSQL_PASSWORD" "$MYSQL_PASSWORD"
    upsert_env_assignment "MYSQL_ROOT_PASSWORD" "$MYSQL_ROOT_PASSWORD"
    upsert_env_assignment "SQLALCHEMY_DATABASE_URL" "mysql+pymysql://next:${mysql_password_url_encoded}@127.0.0.1:3306/next"
    disable_managed_database_binary_log
}
...
configure_binary_database() {
    ...
    case "$database_type" in
        sqlite|"")
            upsert_env_assignment "NEXT_DATABASE_FLAVOR" "sqlite"
...
install_command() {
    ...
    # Check if next is already installed
    if is_next_installed; then
        colorized_echo red "Next is already installed at $APP_DIR"
    ...
    if [[ "$next_version_set" != "true" ]]; then
        next_version=$(select_next_version "" "$install_mode")
    fi
    set_next_source_for_version "$next_version"
    detect_os
    if ! command -v jq >/dev/null 2>&1; then
    ...
    install_next_script "$next_version"
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/${NEXT_RELEASE_REPO}/releases"
    ...
    if [[ "$next_version" == "latest" || "$next_version" == "dev" || "$next_version" =~ ^dev-[0-9a-fA-F]{7,40}$ || "$next_version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        if check_version_exists "$next_version"; then
            if [ "$install_mode" = "binary" ]; then
                install_binary_next "$next_version" "$database_type"
                prompt_dashboard_bind_settings
                prompt_initial_admin
                ...
                install_next "$next_version" "$database_type"
                echo "docker" > "$INSTALL_MODE_FILE"
            fi
            write_next_channel "$next_version"
            echo "Installing $next_version version"
        else
            echo "Version $next_version does not exist. Please enter a valid version (e.g. v0.5.2)"
            exit 1
        fi
    else
...
show_next_logs() {
    if is_binary_install; then
        journalctl -u "$APP_NAME.service" -o "$(journal_output_format)" --no-pager | format_next_journal_logs
        return
    fi
...
next_cli() {
    if is_binary_install; then
        NEXT_ENV_FILE="$ENV_FILE" NEXT_APP_DIR="$APP_DIR" NEXT_DATA_DIR="$DATA_DIR" CLI_PROG_NAME="next cli" "$BINARY_CLI" "$@"
        return
    fi

    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" exec -e CLI_PROG_NAME="next cli" next next-cli "$@"
}
...
uninstall_command() {
    ...
    if [ "$app_exists" -eq 0 ]; then
        colorized_echo red "Next's not installed!"
        exit 1
    fi

    read -p "Do you really want to uninstall Next? (y/n) "
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    ...
    uninstall_next_script

    if [ "$app_exists" -eq 1 ]; then
        uninstall_next
        if [ "$install_mode" != "binary" ]; then
            uninstall_next_docker_images
        fi

        read -p "Do you want to remove Next's data files too ($DATA_DIR)? (y/n) "
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            colorized_echo green "Next uninstalled successfully"
        else
            uninstall_next_data_files
            colorized_echo green "Next uninstalled successfully"
        fi
    else
        colorized_echo green "Legacy Next script removed"
    fi
}

uninstall_next_script() {
    if [ -f "/usr/local/bin/next" ]; then
        colorized_echo yellow "Removing next script"
...
uninstall_next_docker_images() {
    ...
    images=$(docker images | grep next | awk '{print $3}')
    
    if [ -n "$images" ]; then
        colorized_echo yellow "Removing Docker images of Next"
...
restart_command() {
    help() {
        colorized_echo red "Usage: next restart [options]"
        echo
        echo "OPTIONS:"
    ...
    if is_binary_install; then
        ...
        if [ "$no_logs" = true ]; then
            ...
            colorized_echo green "Next restart scheduled."
            return
        fi
        restart_binary_service_now
        follow_next_logs
    ...
    colorized_echo green "Next successfully restarted!"
}
logs_command() {
    help() {
        colorized_echo red "Usage: next logs [options]"
        echo ""
        echo "OPTIONS:"
        echo "  -h, --help        display this help message"
    ...
    if ! is_next_up; then
        colorized_echo red "Next is not up."
        exit 1
    fi
...
down_command() {
    ...
    if ! is_next_up; then
        colorized_echo red "Next's already down"
        exit 1
    fi
...
up_command() {
    help() {
        colorized_echo red "Usage: next up [options]"
        echo ""
        echo "OPTIONS:"
        echo "  -h, --help        display this help message"
        echo "  -n, --no-logs     do not follow logs after starting"
    ...
    if is_next_up; then
        colorized_echo red "Next's already up"
        exit 1
    fi
...
update_command() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --version)
                ...
                ;;
            -h|--help)
                colorized_echo red "Usage: next update [--dev | --version vX.Y.Z|dev-abcdef0]"
                exit 0
                ;;
            *)
                colorized_echo red "Unknown option: $1"
    ...
    if [[ "$next_version_set" != "true" ]]; then
        next_version=$(get_installed_next_channel)
    fi
    set_next_source_for_version "$next_version"

    if ! is_binary_install; then
        if [[ "$next_version" =~ ^dev-[0-9a-fA-F]{7,40}$ ]]; then
    ...
    colorized_echo blue "Updating Next CLI..."
    update_next_script "$next_version"

    colorized_echo blue "Updating requested version: $next_version"
    update_next "$next_version"
    if is_binary_install && [ -d /usr/share/phpmyadmin ]; then
        configure_phpmyadmin_upload_limits
    fi
    write_next_channel "$next_version"
    
    colorized_echo blue "Restarting Next's services"
    if is_binary_install; then
        if ! restart_binary_database_if_running; then
            return 1
        fi
        schedule_binary_service_restart 1
        colorized_echo blue "Next updated successfully; restart scheduled."
        return
    fi
    ...
    colorized_echo blue "Next updated successfully"
}
...
update_next_script() {
    ...
    SCRIPT_URL="$NEXT_SCRIPT_BASE_URL/$NEXT_SCRIPT_SOURCE_FILE"
    colorized_echo blue "Updating next script"
    temp_script=$(mktemp)
    ...
    install -m 755 "$temp_script" "$NEXT_SCRIPT_INSTALL_PATH"
    rm -f "$temp_script"
    colorized_echo green "next script updated successfully"
}

set_compose_next_image_tag() {
    local next_version="$1"
    ...
    if [ "$next_version" = "latest" ]; then
        yq -i '.services.next.image = "nextpanel/next:latest"' "$COMPOSE_FILE"
    else
        yq -i ".services.next.image = \"nextpanel/next:${next_version}\"" "$COMPOSE_FILE"
    fi
}

update_next() {
    local next_version="${1:-latest}"

    if is_binary_install; then
        install_binary_next "$next_version" "$(get_configured_database_type)" "0"
        return
    fi

    set_compose_next_image_tag "$next_version"
...
dump_docker_database() {
    ...
    case "$database_type" in
        mysql)
            ...
            docker exec "$container_id" mysqldump -uroot -p"$root_password" --single-transaction --routines --events --triggers next > "$backup_dir/db.sql"
        ;;
        mariadb)
            root_password=$(get_env_value "MYSQL_ROOT_PASSWORD")
            ...
            docker exec "$container_id" sh -c 'command -v mariadb-dump >/dev/null 2>&1 && exec mariadb-dump "$@" || exec mysqldump "$@"' sh -uroot -p"$root_password" --single-transaction --routines --events --triggers next > "$backup_dir/db.sql"
        ;;
        sqlite)
            local sqlite_path
    ...
    case "$database_type" in
        ...
        mysql|mariadb)
            if [ -f "$backup_dir/db.sql" ]; then
                mysql_root_command -e "DROP DATABASE IF EXISTS \`next\`; CREATE DATABASE \`next\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
                mysql_root_command next < "$backup_dir/db.sql"
            fi
        ;;
    esac
}
...
migrate_docker_to_binary_command() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            ;;
            -h|--help)
                colorized_echo red "Usage: next migrate-binary [--database sqlite|mysql|mariadb] [--dev | --version vX.Y.Z] [-y]"
                exit 0
            ;;
            *)
                colorized_echo red "Unknown option: $1"
    ...
    if ! is_next_installed || [ ! -f "$COMPOSE_FILE" ]; then
        colorized_echo red "Docker installation not found at $APP_DIR"
        exit 1
    fi
    if is_binary_install; then
        colorized_echo yellow "Next is already in binary mode."
        exit 0
    fi
    if [ "$next_version_set" != "true" ]; then
        next_version=$(get_installed_next_channel)
    ...
    if [ "$yes" != "true" ]; then
        colorized_echo yellow "This will migrate Next from Docker to binary mode using $database_type."
        read -p "Continue? (y/n) "
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            colorized_echo red "Aborted"
            exit 1
    ...
    detect_compose
    local backup_dir="/opt/next-docker-to-binary-backups/$(date -u +%Y%m%d%H%M%S)"
    mkdir -p "$backup_dir"
    cp "$ENV_FILE" "$backup_dir/.env" 2>/dev/null || true
    cp "$COMPOSE_FILE" "$backup_dir/docker-compose.yml" 2>/dev/null || true
    if command -v rsync >/dev/null 2>&1; then
        rsync -a --exclude mysql --exclude xray-core "$DATA_DIR/" "$backup_dir/next-data/" 2>/dev/null || true
    else
        mkdir -p "$backup_dir/next-data"
        cp -a "$DATA_DIR/." "$backup_dir/next-data/" 2>/dev/null || true
        rm -rf "$backup_dir/next-data/mysql" "$backup_dir/next-data/xray-core" 2>/dev/null || true
    fi

    if [ "$database_type" = "sqlite" ] && is_next_up; then
        down_next
    fi
    if [ "$database_type" != "sqlite" ] && ! is_next_up; then
        up_next
    ...
    colorized_echo blue "Installing Next binary files"
...
menu_description_for() {
    case "$1" in
        ...
        install) echo "Install Next" ;;
        update) echo "Update to latest version" ;;
        uninstall) echo "Uninstall Next" ;;
        script-install) echo "Install Next script" ;;
        script-update) echo "Update Next CLI script" ;;
        script-uninstall) echo "Uninstall Next script" ;;
...
print_menu() {
    ...
    ui_header "Next Panel" "Control center"
    ui_section "Status"
    print_menu_status_summary
    ui_section "Actions"
...
usage() {
    local script_name="${0##*/}"
    colorized_echo blue "=============================="
    colorized_echo magenta "           Next Help"
    colorized_echo blue "=============================="
    ...
    colorized_echo yellow "  cli             - Next CLI"
    colorized_echo yellow "  migrate         - Run database migrations"
    colorized_echo yellow "  install         - Install Next"
    colorized_echo yellow "  update          - Update to latest/dev or a specific release"
    colorized_echo yellow "  uninstall       - Uninstall Next"
    colorized_echo yellow "  script-install  - Install Next script"
    colorized_echo yellow "  script-update   - Update Next CLI script"
    colorized_echo yellow "  script-uninstall  - Uninstall Next script"
    colorized_echo yellow "  backup          - Manual backup launch"
    colorized_echo yellow "  backup-service  - Next Backupservice to backup to TG, and a new job in crontab"
...
dispatch_command() {
    ...
    case "$cmd" in
        ...
        script-install|install-script) install_next_script "$@" ;;
        script-update|update-script) install_next_script "$@" ;;
        script-uninstall|uninstall-script) uninstall_next_script "$@" ;;
