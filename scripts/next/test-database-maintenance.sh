...
SCRIPT_PATH="$SCRIPT_DIR/next-binary.sh"

new_fixture() {
    local root="$1"
    mkdir -p "$root/mysql.conf.d"
    printf '[mysqld]\nbind-address=127.0.0.1\n' > "$root/mysql.conf.d/next.cnf"
}
...
test_dedicated_database() (
    ...
    new_fixture "$root"
    export NEXT_SOURCE_ONLY=1 NEXT_MYSQL_CONFIG_ROOT="$root"
    source "$SCRIPT_PATH"
    ...
    grep -qx 'skip-log-bin' "$root/mysql.conf.d/next.cnf"
    [ "$purged" = "1" ]
    [ "$restarted" = "1" ]
)
...
test_shared_database_is_untouched() (
    ...
    trap 'rm -rf "$root"' EXIT
    new_fixture "$root"
    before=$(cat "$root/mysql.conf.d/next.cnf")
    export NEXT_SOURCE_ONLY=1 NEXT_MYSQL_CONFIG_ROOT="$root"
    source "$SCRIPT_PATH"
    ...
    disable_managed_database_binary_log >/dev/null
    [ "$(cat "$root/mysql.conf.d/next.cnf")" = "$before" ]
)
...
test_external_database_is_rejected() (
    ...
    source "$SCRIPT_PATH"
    get_env_value() { echo 'mysql+pymysql://next:secret@db.example.com:3306/next'; }
