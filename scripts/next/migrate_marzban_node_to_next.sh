...
log() {
    echo -e "\e[96m[next-node-migrate]\e[0m $1"
}

warn() {
    echo -e "\e[93m[next-node-migrate]\e[0m $1"
}

error_exit() {
    echo -e "\e[91m[next-node-migrate]\e[0m $1"
    exit 1
}
...
def replace_paths(value: str) -> str:
    value = re.sub(r'(:\s*["\']?)/var/lib/marzban-node', r'\1/var/lib/next-node', value)
    value = re.sub(r'(:\s*["\']?)/var/lib/marzban', r'\1/var/lib/next', value)
    return value
...
replacements = [
    (re.compile(r'(=\s*["\']?)/var/lib/marzban-node'), r"\1/var/lib/next-node"),
    (re.compile(r'(=\s*["\']?)/var/lib/marzban'), r"\1/var/lib/next"),
]
...
install_node_cli() {
    if ! command -v curl >/dev/null 2>&1; then
        error_exit "curl is required to install the Next-node CLI."
    fi
