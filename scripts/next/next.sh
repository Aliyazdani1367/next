...
CERTBOT_BIN=""
PARSED_DOMAINS=()
NEXT_SCRIPT_FLAVOR="${NEXT_SCRIPT_FLAVOR:-docker}"
NEXT_SCRIPT_SOURCE_FILE="${NEXT_SCRIPT_SOURCE_FILE:-next.sh}"
...
install_host_database() {
    ...
    case "$database_type" in
        mysql)
            ...
            service_name="mysql"
            config_file="/etc/mysql/mysql.conf.d/next.cnf"
        ;;
        mariadb)
            package_name="mariadb-server"
            service_name="mariadb"
            config_file="/etc/mysql/mariadb.conf.d/60-next.cnf"
        ;;
        *)
            return 0
        ;;
