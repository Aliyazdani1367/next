...
NEW_APP_DIR="/opt/next"
OLD_DATA_DIR="/var/lib/marzban"
NEW_DATA_DIR="/var/lib/next"
OLD_SERVICE_NAME="marzban"
NEW_SERVICE_NAME="next"
SCRIPT_URL="https://raw.githubusercontent.com/nextpanel/Next/master/scripts/next/next.sh"

PANEL_IMAGE_REPO="nextpanel/next"
DEFAULT_IMAGE_TAG="latest"
PYTHON_BIN=""

log() {
    echo -e "\e[96m[next-migrate]\e[0m $1"
}

warn() {
    echo -e "\e[93m[next-migrate]\e[0m $1"
}

error_exit() {
    echo -e "\e[91m[next-migrate]\e[0m $1"
    exit 1
}
...
update_file_references() {
    ...
    replace_text_in_file "$file" \
        "/var/lib/marzban" "/var/lib/next" \
        "/opt/marzban" "/opt/next" \
        "Marzban" "Next"
}
...
def replace_paths(value: str) -> str:
    value = value.replace("/var/lib/marzban", "/var/lib/next")
    value = value.replace("/opt/marzban", "/opt/next")
    value = value.replace("Marzban", "Next")
    return value
...
def replace_names(value: str) -> str:
    value = re.sub(r'^(\s*)marzban(\s*:)', r'\1next\2', value, flags=re.MULTILINE)
    value = re.sub(r'(container_name:\s*["\']?)marzban(["\']?)', r'\1next\2', value)
    return value
...
for line in lines:
    ...
    new_value = value
    new_value = new_value.replace("/var/lib/marzban", "/var/lib/next")
    new_value = new_value.replace("/opt/marzban", "/opt/next")
...
install_next_cli() {
    if ! command -v curl >/dev/null 2>&1; then
        error_exit "curl is required to install the Next CLI."
    fi
    if [ -f "/usr/local/bin/next" ]; then
        backup_existing_path "/usr/local/bin/next"
    fi
    curl -sSL "$SCRIPT_URL" | install -m 755 /dev/stdin /usr/local/bin/next
    warn "Old next CLI (if any) can now be removed manually."
    log "Installed Next CLI to /usr/local/bin/next"
}
...
rerun_install_service_script() {
    ...
    systemctl enable --now next.service >/dev/null 2>&1 || true
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            if result:
                ...
                # Replace addresses
                config_str = config_str.replace("/var/lib/marzban", "/var/lib/next")
                config_str = config_str.replace("/opt/marzban", "/opt/next")
...
restart_next_panel() {
    ...
    if command -v next >/dev/null 2>&1; then
        log "All migration steps completed. Handing over to 'next restart' as final step."
        log "From now on it is the same as running: next restart"
        exec next restart
    fi

    if [ ! -f "$compose_file" ]; then
        warn "Cannot find $compose_file to start Next via docker compose."
        return
    fi

    log "Next CLI not found; starting Next stack using docker compose."
    if $compose_bin -f "$compose_file" -p "$NEW_SERVICE_NAME" up -d --remove-orphans; then
        log "Next stack started successfully via docker compose."
    else
...
main() {
    ...
    restart_next_panel "$compose_bin"

    log "Migration complete. You can now manage the panel using the 'next' command."
    log "For example: next up"
}
