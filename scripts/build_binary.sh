...
gateway_output="$ROOT_DIR/dist/next-server"
...
if [[ "${OS:-}" == "Windows_NT" ]]; then
    gateway_output="$ROOT_DIR/dist/next-server.exe"
fi

(
    prepare_go_dashboard_embed
    cd "$ROOT_DIR"
    CGO_ENABLED=0 go build -trimpath -buildvcs=false -o "$gateway_output" ./cmd/next_gateway
)

echo "Next Go gateway built at $gateway_output"
