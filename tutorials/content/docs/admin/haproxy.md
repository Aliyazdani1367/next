...
Next can run HAProxy on selected nodes and use one public TCP port for several destinations. HAProxy reads the beginning of each connection, matches an SNI, HTTP Host, or HTTP Path, then passes the original TCP stream to the correct Xray inbound, external service, or website. It does not decrypt Xray traffic.
...
2. A **target** is a Next node that will run this configuration.
...
10. Open **Editor**. Next validates the draft and shows the exact HAProxy file for the target node.
...
The inbound selector is generated from the selected node's actual Xray configuration. An inbound needs a nonempty tag, a valid port, and a usable matcher. Next derives the matcher from `streamSettings`.

| Inbound transport or security | Matcher shown by Next | Requirement |
| --- | --- | --- |
...
| **Website name** | Internal label, up to 64 characters. If empty, Next uses the hostname. |
...
| **TemplateMo page URL** | Accepts a page URL such as `https://templatemo.com/tm-632-machina`. Next converts it to the matching download, and the node downloads and caches the ZIP. |
...
| **Certificate from panel SSL** | Select an active certificate already managed by Next. The certificate must cover the website hostname. The panel securely sends its PEM data to the selected node during sync. |
...
| **Log level** | `info` | `silent`, `emerg`, `alert`, `crit`, `err`, `warning`, `notice`, `info`, `debug` | Amount of HAProxy output written to the Next Node service logs. Use `debug` only while investigating because it can be noisy. |
...
Next skips a destination if it is disabled, already belongs to another configuration, cannot be inspected, or lacks any required Xray tag and matcher. Generated target cards start collapsed.
...
| Uploaded template does not reach a node | The panel needs a usable `NEXT_PUBLIC_URL`, and the node must have an enrolled certificate so it can fetch the archive. |
...
| HAProxy is not installed | Update or reinstall Next Node. Current binary installation, update, and Docker image include the package. |
...
For node-side errors, inspect Next Node service logs. HAProxy validation, missing package, template download, certificate, and bind failures are reported there. Temporarily use the `debug` log level only when the normal log does not show enough detail.
