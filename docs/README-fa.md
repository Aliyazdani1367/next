...
- فایل‌های رِبِکا در پوشه `/opt/next` قرار می‌گیرند
- فایل تنظیمات در مسیر `/opt/next/.env` قرار می‌گیرد ([تنظیمات](#تنظیمات) را مشاهده کنید)
- فایل‌های مهم رِبِکا در مسیر `/var/lib/next` قرار می‌گیرند
...
| آدرس کامل gateway؛ مقدار `UVICORN_HOST` و `UVICORN_PORT` را override می‌کند | NEXT_GATEWAY_ADDR |
| فاصله پردازش صف node operations | NEXT_NODE_OPERATIONS_POLL_INTERVAL |
| فاصله بررسی lifecycle کاربران | NEXT_USER_LIFECYCLE_INTERVAL |
| فاصله reset دوره‌ای مصرف کاربران | NEXT_USER_USAGE_RESET_INTERVAL |
...
| مسیر پایه certificateهای مدیریت‌شده | NEXT_CERT_BASE |
| ریشه configهایی که در full backup قرار می‌گیرند | NEXT_CONFIG_DIR |
...
لطفاً با بررسی [لیست کارها](https://github.com/nextpanel/Next/issues) به ما در بهبود رِبِکا کمک کنید. کمک‌های شما با آغوش باز پذیرفته می‌شود.
