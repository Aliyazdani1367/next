...
Next is a proxy management tool that provides a simple and easy-to-use user interface for managing hundreds of proxy accounts powered by [Xray-core](https://github.com/XTLS/Xray-core) and built with a Go backend and React dashboard.

## Why use Next?

Next is user-friendly, feature-rich and reliable. It lets you create different proxies for your users without any complicated configuration. Using its built-in web UI, you can monitor, modify and limit users.
...
- The data files will be placed at `/var/lib/next`
- For security reasons, the Next dashboard is not accessible via IP address. Therefore, you must obtain an SSL certificate and access your Next dashboard by opening a web browser and navigating to `https://YOUR_DOMAIN:8000/dashboard/` (replace YOUR_DOMAIN with your actual domain)
...
Finally, you can enter the following link in your browser to access your Next dashboard:
...
> **Full access admins** can only be created by another full access admin from the dashboard or by using the on-box `next cli`.  
> If you ever need to promote an existing sudo admin, run  
> `next cli admin change-role --username YOUR_ADMIN --role full_access`

To see the help message of the Next script, run the following command
...
```bash
./dist/next-cli migrate up
```

Downgrade migrations are not supported. Check the migration status with
`./dist/next-cli migrate status` before upgrading a legacy database.
...
server {
  ...
  # The '/next' in the location regex path can be changed to any value you like.
    #
...
| NEXT_GATEWAY_ADDR                     | Optional full gateway listen address. Overrides `UVICORN_HOST`/`UVICORN_PORT`.                                          |
| NEXT_NODE_OPERATIONS_POLL_INTERVAL    | Node operation queue polling interval.                                                                                   |
| NEXT_USER_LIFECYCLE_INTERVAL          | User lifecycle review interval.                                                                                          |
| NEXT_USER_USAGE_RESET_INTERVAL        | Periodic user usage reset interval.                                                                                      |
| NEXT_USER_AUTODELETE_INTERVAL         | Expired/limited user auto-delete job interval.                                                                            |
...
| NEXT_CERT_BASE                        | Base directory for managed certificates.                                                                                  |
| NEXT_CERTBOT_BIN                      | Optional Certbot executable override used by the SSL manager.                                                              |
| NEXT_CONFIG_DIR                       | Configuration root included in full backup export/import.                                                                |
...
If you found Next useful and would like to support its development, you can make a donation in one of the following crypto networks:
...
Check [open issues](https://github.com/aliking1367/next/issues) to help the progress of this project.
