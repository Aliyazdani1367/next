...
import (
	...
	"github.com/nextpanel/next/internal/app/api"
	"github.com/nextpanel/next/internal/app/logging"
	"github.com/nextpanel/next/internal/gateway"
)
...
func main() {
	...
	if err != nil {
		logging.Fatalf(logging.ComponentRuntime, "failed to load Next API config: %v", err)
	}
	api, err := api.New(apiCfg)
	if err != nil {
		logging.Fatalf(logging.ComponentRuntime, "failed to initialize Next API: %v", err)
	}
