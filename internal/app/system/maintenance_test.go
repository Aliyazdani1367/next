...
func TestGitHubUpdateCheckerCachesSuccessfulStatus(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/repos/nextpanel/Next/releases/latest":
			_ = json.NewEncoder(w).Encode(map[string]any{
				"tag_name":     "v0.2.0",
				"name":         "v0.2.0",
				"published_at": "2026-06-24T00:00:00Z",
			})
		case "/nextpanel/Next/dev-build-manifest/dev-builds.json":
			_ = json.NewEncoder(w).Encode(map[string]any{
	...
	first := checker.Status(context.Background(), "nextpanel/Next", &current, "dev")
	second := checker.Status(context.Background(), "nextpanel/Next", &current, "dev")
...
func TestGitHubUpdateCheckerFindsDevBuildThroughWorkflowEndpoint(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/nextpanel/Next/dev-build-manifest/dev-builds.json":
			http.NotFound(w, r)
		case "/repos/nextpanel/Next/actions/workflows/binary-build.yml/runs":
			query := r.URL.Query()
	...
	current := "dev-0000000"
	status := checker.Status(context.Background(), "nextpanel/Next", &current, "dev")
...
func TestGitHubUpdateCheckerCachesErrors(t *testing.T) {
	...
	first := checker.Status(context.Background(), "nextpanel/Next", nil, "latest")
	second := checker.Status(context.Background(), "nextpanel/Next", nil, "latest")
