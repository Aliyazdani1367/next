...
func (s *MaintenanceService) Info(ctx context.Context) (MaintenanceInfo, error) {
	...
	panel.Update = ptr(s.Updates.Status(ctx, "nextpanel/Next", panel.Tag, panel.Channel))
	return MaintenanceInfo{
		Panel:      panel,
		Node:       nil,
		NodeUpdate: s.Updates.Status(ctx, "nextpanel/Next-node", nil, ""),
	}, nil
}
...
func (s *MaintenanceService) Update(_ context.Context, req MaintenanceUpdateRequest) (MaintenanceOperationSnapshot, error) {
	...
	args, err := BuildNextUpdateArgs(req.Channel, req.Version)
	if err != nil {
		return MaintenanceOperationSnapshot{}, err
	}
...
func (s *MaintenanceService) startOperation(action string, args []string, message string) (MaintenanceOperationSnapshot, error) {
	...
	if action == "restart" || action == "soft-reload" {
		...
		s.ops.MarkRestarting(op.ID, "Command accepted. Waiting for Next to restart.")
...
func BuildNextUpdateArgs(channel string, version string) ([]string, error) {
	args := []string{"update"}
...
func (DefaultRuntimeDetector) Info() RuntimeInfo {
	...
	mode := strings.ToLower(strings.TrimSpace(os.Getenv("NEXT_INSTALL_MODE")))
	if mode == "" {
...
func serviceName() string {
	if value := strings.TrimSpace(os.Getenv("NEXT_SERVICE_NAME")); value != "" {
		return value
	}
	return "next"
}
...
func runtimeMetadataPath() string {
	if value := strings.TrimSpace(os.Getenv("NEXT_BINARY_METADATA_FILE")); value != "" {
		return value
	}
...
func installModePath() string {
	if value := strings.TrimSpace(os.Getenv("NEXT_INSTALL_MODE_FILE")); value != "" {
		return value
	}
...
func appDir() string {
	if value := strings.TrimSpace(os.Getenv("NEXT_APP_DIR")); value != "" {
		return value
	}
	return "/opt/next"
}
...
func (DefaultCommandScheduler) Schedule(args []string) error {
	...
	if runtime.GOOS != "windows" {
		if systemdRun, err := exec.LookPath("systemd-run"); err == nil && systemdRun != "" {
			unit := fmt.Sprintf("next-host-action-%d", time.Now().UnixNano())
			command = append(
				[]string{systemdRun, "--unit", unit, "--collect", "--description", "Next host action", "--"},
				command...,
			)
		}
	}
	...
	if err := cmd.Start(); err != nil {
		return MaintenanceError{Status: http.StatusInternalServerError, Detail: "Failed to schedule Next command: " + err.Error()}
	}
	if cmd.Process != nil {
...
func (DefaultCommandScheduler) ScheduleWithProgress(args []string, onOutput func(string), onDone func(error)) error {
	...
	if err != nil {
		return MaintenanceError{Status: http.StatusInternalServerError, Detail: "Failed to capture Next command output: " + err.Error()}
	}
...
func resolveNextCLI() (string, error) {
	candidates := []string{strings.TrimSpace(os.Getenv("NEXT_SCRIPT_BIN"))}
	if path, err := exec.LookPath("next"); err == nil {
		candidates = append(candidates, path)
	}
	candidates = append(candidates, "/usr/local/bin/next")
	...
	return "", MaintenanceError{Status: http.StatusServiceUnavailable, Detail: "Next CLI was not found on this host"}
}
...
func NewGitHubUpdateChecker() *GitHubUpdateChecker {
	return &GitHubUpdateChecker{
		...
		ManifestBranch: firstEnv("NEXT_BINARY_DEV_MANIFEST_BRANCH", "dev-build-manifest"),
...
func (c *GitHubUpdateChecker) getJSON(ctx context.Context, url string, target any) error {
	...
	req.Header.Set("User-Agent", "Next-update-check")
