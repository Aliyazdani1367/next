...
const phpMyAdminEmbedCookie = "next_pma_embed"
...
func (s *Server) handlePHPMyAdminEnable(w http.ResponseWriter, r *http.Request) {
	...
	output, err := runNextPHPMyAdminCommand(r.Context(), "enable-phpmyadmin", "--path", path)
	if err != nil {
...
func (s *Server) handlePHPMyAdminDisable(w http.ResponseWriter, r *http.Request) {
	...
	output, err := runNextPHPMyAdminCommand(r.Context(), "disable-phpmyadmin")
	if err != nil {
		writeError(w, http.StatusBadGateway, strings.TrimSpace(output+"\n"+err.Error()))
		return
	}
...
func ensurePHPMyAdminRuntimeConfig(credentials phpMyAdminCredentials, theme string) error {
	...
	config := "<?php\n" +
		...
		"$cfg['ExecTimeLimit'] = 0;\n" +
		themeLine
	if err := os.WriteFile("/etc/phpmyadmin/conf.d/next.php", []byte(config), 0o644); err != nil {
		return fmt.Errorf("write phpMyAdmin runtime config: %w", err)
	}
	if err := os.Chmod("/etc/phpmyadmin/conf.d/next.php", 0o644); err != nil {
...
func runNextPHPMyAdminCommand(parent context.Context, args ...string) (string, error) {
	ctx, cancel := context.WithTimeout(parent, 10*time.Minute)
	defer cancel()
	binary := "next"
	if _, err := os.Stat("/usr/local/bin/next"); err == nil {
...
<body>
<div class="loading" id="loading"><div class="panel"><strong>Opening phpMyAdmin...</strong><p class="muted">Next is signing in with the configured database account for this full-access session.</p></div></div>
<iframe id="pma-frame" src="` + escapedLoginURL + `" title="phpMyAdmin" onload="nextRevealPHPMyAdmin()"></iframe>
<script>
` + themeCookieScript + `
function nextRevealPHPMyAdmin(){
...
var nextPMARevealAttempts=0;
var nextPMARevealTimer=setInterval(function(){
  nextRevealPHPMyAdmin();
  nextPMARevealAttempts++;
  if (nextPMARevealAttempts > 40) clearInterval(nextPMARevealTimer);
}, 250);
</script>
</body>
</html>`
...
func newPHPMyAdminEmbedSecret() []byte {
	...
	return []byte(fmt.Sprintf("next-pma-%d", time.Now().UnixNano()))
}
