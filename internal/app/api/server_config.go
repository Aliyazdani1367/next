...
func LoadConfig() (Config, error) {
	...
	cfg := Config{
		...
		CertificateBase:              lookup("NEXT_CERT_BASE"),
		CertbotBinary:                lookup("NEXT_CERTBOT_BIN"),
		ExternalAppsBase:             lookup("NEXT_EXTERNAL_APPS_BASE"),
		MySQLRootPassword:            lookup("MYSQL_ROOT_PASSWORD"),
		NodeOperationsPollInterval:   lookup("NEXT_NODE_OPERATIONS_POLL_INTERVAL"),
		NodeUsageCollectionInterval:  lookup("NEXT_NODE_USAGE_COLLECTION_INTERVAL"),
		NodeUsageCollectionLimit:     parseIntDefault(lookup("NEXT_NODE_USAGE_COLLECTION_LIMIT"), 0),
		NodeUsageFlushInterval:       lookup("NEXT_NODE_USAGE_FLUSH_INTERVAL"),
		...
		UserUsageResetInterval:       lookup("NEXT_USER_USAGE_RESET_INTERVAL"),
		UserUsageResetBatchSize:      parseIntDefault(lookup("NEXT_USER_USAGE_RESET_BATCH_SIZE"), 500),
		UserAutodeleteInterval:       lookup("NEXT_USER_AUTODELETE_INTERVAL"),
		UserAutodeleteBatchSize:      parseIntDefault(lookup("NEXT_USER_AUTODELETE_BATCH_SIZE"), 500),
		...
		TelegramAPIBase:              lookup("NEXT_TELEGRAM_API_BASE"),
		WebhookAddresses:             splitWebhookAddresses(lookup("WEBHOOK_ADDRESS")),
		WebhookSecret:                lookup("WEBHOOK_SECRET"),
		WebhookSendInterval:          firstNonEmpty(lookup("NEXT_WEBHOOK_SEND_INTERVAL"), secondsEnv(lookup("JOB_SEND_NOTIFICATIONS_INTERVAL"))),
		WebhookMaxRetries:            parseIntDefault(lookup("NUMBER_OF_RECURRENT_NOTIFICATIONS"), 3),
		WebhookRetryInterval:         firstNonEmpty(lookup("NEXT_WEBHOOK_RETRY_INTERVAL"), secondsEnv(lookup("RECURRENT_NOTIFICATIONS_TIMEOUT"))),
	}
	if cfg.Database == "" {
...
func candidateEnvFiles() []string {
	...
	paths := []string{}
	paths = add(paths, os.Getenv("NEXT_ENV_FILE"))
