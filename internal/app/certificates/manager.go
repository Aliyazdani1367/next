...
const (
	DefaultBaseDir          = "/var/lib/next/certificates"
...
func (m *Manager) certbot() (string, error) {
	...
	for _, candidate := range []string{"/opt/next/certbot-venv/bin/certbot", "certbot"} {
		if path, err := exec.LookPath(candidate); err == nil {
			return path, nil
		}
	}
	return "", fmt.Errorf("certbot is not installed; update Next or install certbot first")
}
