...
func TestImportRejectsUnsafeArchivePath(t *testing.T) {
	...
	service := NewService(nil, "sqlite", "sqlite:///tmp/next.sqlite3")
...
func TestMySQLMissingDumpTool(t *testing.T) {
	...
	service := NewService(nil, "mysql", "mysql://user:pass@127.0.0.1:3306/next")
...
func TestFullRestoreKeepsDestinationMySQLCredentials(t *testing.T) {
	targetEnv := filepath.Join(t.TempDir(), "next_env")
	if err := os.WriteFile(targetEnv, []byte(strings.Join([]string{
		`NEXT_DATABASE_FLAVOR="mysql"`,
		`MYSQL_DATABASE="next"`,
		`MYSQL_USER="next"`,
		`MYSQL_PASSWORD="destination-password"`,
		`MYSQL_ROOT_PASSWORD="destination-root-password"`,
		`SQLALCHEMY_DATABASE_URL="mysql+pymysql://next:destination-password@127.0.0.1:3306/next"`,
	}, "\n")+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	filesDir := t.TempDir()
	sourceEnv := filepath.Join(filesDir, "next_env")
	if err := os.WriteFile(sourceEnv, []byte(strings.Join([]string{
		...
		`SQLALCHEMY_DATABASE_URL="mysql+pymysql://next:source-password@127.0.0.1:3306/next"`,
	}, "\n")+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	service := NewService(nil, "mysql", "mysql+pymysql://next:destination-password@127.0.0.1:3306/next", WithFileRoots([]FileRoot{
		{ArchiveName: "next_env", Path: targetEnv},
	}))
...
func TestMySQLAccessDeniedBackupError(t *testing.T) {
	err := mysqlBackupCommandError("restore", "ERROR 1045 (28000): Access denied for user 'next'@'localhost'")
