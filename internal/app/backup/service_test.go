...
func TestExportImportFullBackupFileRoots(t *testing.T) {
	...
	service := NewService(db, "sqlite", sqliteURL(dbPath), WithFileRoots([]FileRoot{
		{ArchiveName: "etc_next", Path: configRoot},
		{ArchiveName: "var_lib_next", Path: dataRoot},
		{ArchiveName: "next_env", Path: envFile},
	}))
...
func TestMySQLBackupURLParsing(t *testing.T) {
	service := NewService(nil, "mysql", "mysql+pymysql://next:p%40ss%21%23frag%3Deq%5Cslash@127.0.0.1:3306/next")
...
func TestFilterMySQLDumpForDatabase(t *testing.T) {
	...
	content := strings.Join([]string{
		...
		"/*!40000 DROP DATABASE IF EXISTS `next`*/;",
		"CREATE DATABASE /*!32312 IF NOT EXISTS*/ `next` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;",
		"USE `next`;",
	...
	for _, forbidden := range []string{"DROP DATABASE", "CREATE DATABASE", "USE `next`"} {
