...
func (s *Service) Export(ctx context.Context, scope string) (ExportResult, error) {
	...
	output, err := os.CreateTemp("", "next-backup-*"+Extension)
	if err != nil {
		return ExportResult{}, err
	}
	...
	buildDir, err := os.MkdirTemp("", "next-backup-build-*")
	if err != nil {
	...
	return ExportResult{
		Path:     outputPath,
		Filename: fmt.Sprintf("next-%s-%s%s", scope, timestamp, Extension),
		Scope:    scope,
	}, nil
}
...
func (s *Service) Import(ctx context.Context, archivePath string) (ImportResult, error) {
	...
	extractDir, err := os.MkdirTemp("", "next-backup-import-*")
	if err != nil {
...
func (s *Service) exportDatabasePayload(ctx context.Context, buildDir string) (databasePayload, error) {
	...
	default:
		return databasePayload{}, Error{Message: "Unsupported database backend for Next backup: " + s.dialect}
	}
}
...
func (s *Service) exportMySQL(ctx context.Context, outputPath string) error {
	...
	tempDir, err := os.MkdirTemp("", "next-mysql-dump-*")
	if err != nil {
		return err
	}
...
func (s *Service) restoreMySQL(ctx context.Context, payloadPath string) error {
	...
	tempDir, err := os.MkdirTemp("", "next-mysql-restore-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tempDir)
...
func mysqlBackupCommandError(action string, stderr string) error {
	...
	if strings.Contains(lower, "error 1045") || strings.Contains(lower, "access denied for user") {
		return Error{Message: "MySQL/MariaDB rejected Next's configured database credentials (error 1045). Check MYSQL_PASSWORD and SQLALCHEMY_DATABASE_URL."}
	}
	if detail == "" {
...
func (s *Service) tableNames(ctx context.Context) ([]string, error) {
	...
	default:
		return nil, Error{Message: "Unsupported database backend for Next backup: " + s.dialect}
	}
	if err != nil {
		return nil, err
	}
...
func (s *Service) restoreFileRoots(filesDir string) ([]string, []string, error) {
	...
	for _, root := range s.fileRoots {
		...
		if allowed[target] != root.ArchiveName {
			return nil, nil, Error{Message: "Refusing to restore outside Next paths: " + root.Path}
		}
...
func (s *Service) preserveLocalDatabaseEnv(filesDir string) error {
	...
	assignments = upsertEnvAssignment(assignments, dotenvAssignment("NEXT_DATABASE_FLAVOR", s.dialect))
...
func isDatabaseEnvKey(key string) bool {
	return key == "NEXT_DATABASE_FLAVOR" || key == "SQLALCHEMY_DATABASE_URL" || key == "DATABASE_URL" ||
...
func defaultFileRoots() []FileRoot {
	configDir := strings.TrimSpace(os.Getenv("NEXT_CONFIG_DIR"))
	if configDir == "" {
		configDir = "/etc/next"
	}
	...
	roots := []FileRoot{
		{ArchiveName: "etc_next", Path: configDir},
		{ArchiveName: "var_lib_next", Path: dataDir},
	}
	if envFile := defaultEnvFileRoot(); envFile != "" {
		roots = append(roots, FileRoot{ArchiveName: "next_env", Path: envFile})
	}
	return roots
}
...
func defaultEnvFileRoot() string {
	for _, candidate := range []string{
		strings.TrimSpace(os.Getenv("NEXT_ENV_FILE")),
		"/opt/next/.env",
	} {
		if candidate == "" {
			continue
...
func decodeLegacyValue(value any) any {
	...
	marker, _ := object["__next_type__"].(string)
	raw := object["value"]
	switch marker {
	case "bytes":
