...
import (
	...
	platformdb "github.com/nextpanel/next/internal/platform/db"
...
func TestRunMigrationsExternalDatabase(t *testing.T) {
	url := strings.TrimSpace(os.Getenv("NEXT_TEST_DATABASE_URL"))
	if url == "" {
		t.Skip("NEXT_TEST_DATABASE_URL is not set")
	}
	ctx := context.Background()
