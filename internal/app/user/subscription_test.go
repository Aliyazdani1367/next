...
import (
	...
	"github.com/nextpanel/next/internal/app/outboundsub"
)
...
func TestShadowsocksTLSUsesClientNativeLinkWithoutLossyConversion(t *testing.T) {
	...
	if binary := strings.TrimSpace(os.Getenv("NEXT_XRAY_TEST_BINARY")); binary != "" {
...
func TestStructuredSubscriptionsCoverSupportedShareProtocols(t *testing.T) {
	...
	if binary := strings.TrimSpace(os.Getenv("NEXT_SING_BOX_TEST_BINARY")); binary != "" {
		path := filepath.Join(t.TempDir(), "config.json")
...
func TestSingBoxSubscriptionUsesFullTemplateAndRealNames(t *testing.T) {
	...
	settings := `{"wsSettings":{"headers":{"User-Agent":"Next"}}}`
	...
	for _, raw := range config["outbounds"].([]any) {
		...
		case "vless":
			...
			if headers["User-Agent"] != "Next" || headers["Host"] == "" {
	...
	for _, service := range []struct {
		...
		for _, template := range []struct {
			...
			t.Run(service.name+"/"+template.name, func(t *testing.T) {
				...
				if binary := strings.TrimSpace(os.Getenv("NEXT_XRAY_VLESS_ENCRYPTION_TEST_BINARY")); binary != "" {
...
func TestGenericXrayJSONStableDialectAcceptedByOfficialXray(t *testing.T) {
	binary := strings.TrimSpace(os.Getenv("NEXT_XRAY_STABLE_TEST_BINARY"))
	if binary == "" {
		t.Skip("set NEXT_XRAY_STABLE_TEST_BINARY to the official stable Xray binary")
	}
...
func TestGenericXrayJSONCurrentDialectAcceptedByOfficialXray(t *testing.T) {
	binary := strings.TrimSpace(os.Getenv("NEXT_XRAY_CURRENT_TEST_BINARY"))
	if binary == "" {
		t.Skip("set NEXT_XRAY_CURRENT_TEST_BINARY to the official current Xray binary")
	}
	fm := url.QueryEscape(`{"tcp":[{"type":"fragment","settings":{"packets":"tlshello","lengths":["3-5"],"delays":["10-20"],"maxSplit":"3"}}]}`)
