...
var (
	...
	errExternalAppProtectedPath = errors.New("this application path is managed by Next")
...
func protectedExternalAppSegment(segment string) bool {
	...
	case ".composer", ".git", ".locks", ".logs", ".next", ".npm", ".sessions", ".tmp", "node_modules", "vendor":
		return true
	default:
		return strings.HasPrefix(segment, ".next-")
	}
}
...
func writeExternalAppFile(ctx context.Context, root *os.Root, record Record, name string, data []byte, overwrite bool) error {
	...
	tempName := path.Join(path.Dir(name), ".next-write-"+suffix)
...
func (m *Manager) safePHPConfigRecord(domain string) (Record, error) {
	...
	expected := filepath.Join("/etc/php", record.PHPVersion, "fpm", "pool.d", "next-"+record.ID+".conf")
...
func validateExternalAppPoolConfig(record Record, content string) error {
	...
	for key, expectedValue := range expected {
		...
		if validate, editableKey := editable[key]; editableKey {
			...
			return fmt.Errorf("%w: %s is managed by Next", errExternalAppInvalidConfig, key)
		}
	}
