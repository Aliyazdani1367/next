...
exit 0`)
	...
	t.Setenv("NEXT_FAKE_RUNUSER_LOG", runuserLog)
	...
	cronSecret, err := os.ReadFile(filepath.Join(root, ".next-cron-secret"))
	...
	if err != nil || !strings.Contains(string(commands), "php .next-init.php") {
...
func TestMirzaRequestSecretsAreIndependent(t *testing.T) {
	...
	cron.Header.Set("X-Next-Cron-Secret", "webhook-secret")
	...
	cron.Header.Set("X-Next-Cron-Secret", "cron-secret")
	if err := manager.AuthorizeMirzaRequest(cron, record, "cronbot/statusday.php"); err != nil {
		t.Fatal(err)
	}
}
...
func TestWriteMirzaCronUsesPerAppIdentityWithoutEmbeddingSecret(t *testing.T) {
	...
	record := Record{
		...
		CronConfig: filepath.Join(t.TempDir(), "next-php-abcdef"),
	}
	...
	if !strings.Contains(text, "rbphp_abcdef") || !strings.Contains(text, ".next-cron-secret") {
...
func TestWriteExternalAppPoolBoundsResources(t *testing.T) {
	record := Record{
		Root:       t.TempDir(),
		SystemUser: "rbphp_abcdef",
		Socket:     "/run/php/next-abcdef.sock",
		PoolConfig: filepath.Join(t.TempDir(), "next-abcdef.conf"),
	}
...
func TestCreateExternalAppDatabaseGrantsNextAccess(t *testing.T) {
	bin := t.TempDir()
	logPath := filepath.Join(t.TempDir(), "mysql.log")
	script := `#!/bin/sh
query=$(cat)
printf '%s\n' "$query" >> "$NEXT_MYSQL_TEST_LOG"
case "$query" in
...
`
	...
	t.Setenv("PATH", bin+":"+os.Getenv("PATH"))
	t.Setenv("NEXT_MYSQL_TEST_LOG", logPath)
	manager := &Manager{databaseURL: "mysql://next:secret@127.0.0.1:3306/next"}
	...
	for _, expected := range []string{
		...
		"GRANT ALL PRIVILEGES ON `project_db`.* TO 'next'@'127.0.0.1'",
		"GRANT ALL PRIVILEGES ON `project_db`.* TO 'next'@'localhost'",
	} {
...
func TestDownloadMirzaBotUsesLatestTaggedCommit(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("User-Agent") != "Next" {
...
func TestLatestMirzaBotReleaseArchive(t *testing.T) {
	if os.Getenv("NEXT_TEST_LATEST_MIRZABOT") != "1" {
		t.Skip("set NEXT_TEST_LATEST_MIRZABOT=1 to verify the current tagged GitHub release")
	}
